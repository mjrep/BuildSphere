...
export default function EmptyTaskState({ message = 'No tasks found.', onAdd, canAdd }) {
    return (
        <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <p className="text-text-muted font-medium text-sm mb-1">{message}</p>
            <p className="text-[#A0A0C0] text-xs mb-4">Try adjusting your filters or create a new task.</p>
            {canAdd && onAdd && (
                <button
                    onClick={onAdd}
                    className="px-4 py-2 bg-accent text-white text-sm font-medium rounded-lg hover:opacity-90 transition-colors"
                >
                    + Add Task
                </button>
            )}
