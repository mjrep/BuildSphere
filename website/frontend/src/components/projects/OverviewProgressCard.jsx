...
export default function OverviewProgressCard({ progress }) {
    return (
        <div className="bg-card rounded-2xl shadow-sm border border-border-primary p-6 lg:p-8 w-full flex flex-col justify-center relative">
            <div className="flex justify-between items-start mb-6 w-full absolute top-6 left-6 pr-12">
                <h3 className="text-base font-bold text-text-primary">Project Progress</h3>
                <p className="text-[10px] text-text-muted mt-0.5 whitespace-nowrap">as of {new Date().toLocaleDateString('en-US', { month: '2-digit', day:'2-digit', year:'2-digit' })}</p>
            </div>
