...
export default function AiAssessmentModal({ assessment, onClose }) {
    ...
    // The assessment might be wrapped in ai_assessment or passed directly
    const ai_assessment = assessment.ai_assessment || assessment;
    if (!ai_assessment || !ai_assessment.project_status) return null;
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            {/* Modal Content */}
            <div className="relative bg-card w-full max-w-2xl rounded-[2rem] shadow-2xl overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-4 duration-300">
                ...
                <div className="bg-gradient-to-r from-[#706BFF] to-[#A39FFF] p-8 text-white relative">
                    ...
                    <button 
                        onClick={onClose}
                        className="absolute top-8 right-8 p-2 hover:bg-card/10 rounded-full transition-colors"
                    >
                ...
                <div className="p-8 max-h-[70vh] overflow-y-auto custom-scrollbar">
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-8">
                        <div className="bg-[#F8F8FF] rounded-2xl p-5 border border-border-primary">
                            <span className="text-[10px] uppercase tracking-widest font-bold text-text-muted mb-1 block">Project Status</span>
                            <div className="flex items-center gap-2">
                                <div className={`w-2 h-2 rounded-full ${ai_assessment.project_status === 'Delayed' ? 'bg-red-500' : 'bg-green-500'}`} />
                                <span className="text-lg font-bold text-text-primary">{ai_assessment.project_status}</span>
                            </div>
                        </div>
                        <div className="bg-[#F8F8FF] rounded-2xl p-5 border border-border-primary">
                            <span className="text-[10px] uppercase tracking-widest font-bold text-text-muted mb-1 block">Risk Level</span>
                    ...
                    {/* Executive Summary */}
                    <div className="mb-8">
                        <h3 className="text-xs uppercase tracking-widest font-bold text-text-muted mb-3">Executive Summary</h3>
                        <p className="text-text-primary leading-relaxed text-[15px] font-medium bg-accent/5 p-5 rounded-2xl border border-[#706BFF]/10 italic">
                    ...
                    {/* Suggested Actions */}
                    <div className="mb-8">
                        <h3 className="text-xs uppercase tracking-widest font-bold text-text-muted mb-4">Strategic Recommendations</h3>
                        <div className="space-y-3">
                            {ai_assessment.suggested_actions.map((action, i) => (
                                <div key={i} className="flex gap-4 p-4 bg-card border border-border-primary rounded-xl shadow-sm hover:shadow-md transition-shadow">
                                    ...
                                    <p className="text-sm text-text-secondary font-medium pt-1.5">{action}</p>
                                </div>
                            ))}
                        </div>
                    </div>
                            ...
                            <div className="space-y-3">
                                {ai_assessment.milestone_flags.map((flag, i) => (
                                    <div key={i} className="p-4 rounded-xl border border-border-primary bg-gray-50/50">
                                        ...
                                        <p className="text-sm font-bold text-text-primary mb-1">{flag.milestone_name}</p>
                ...
                {/* Footer */}
                <div className="p-6 border-t border-border-primary bg-[#F8F8FF] flex justify-end">
                    <button 
                        onClick={onClose}
