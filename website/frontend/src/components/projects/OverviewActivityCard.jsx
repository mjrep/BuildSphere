...
export default function OverviewActivityCard({ project }) {
    ...
    return (
        <div className="bg-card rounded-2xl shadow-sm border border-border-primary p-5 w-full flex flex-col flex-1 min-h-0">
            <h3 className="text-base font-bold text-text-primary mb-4 flex-shrink-0">Activity Feed</h3>
            <div className="flex flex-col gap-6 relative overflow-y-auto overflow-x-hidden pr-2 scrollbar-thin scrollbar-thumb-border-primary scrollbar-track-transparent flex-1 min-h-0">
                {/* Vertical Line */}
                {activities.length > 1 && (
                    <div className="absolute left-4 top-2 bottom-2 w-px bg-bg-tertiary -translate-x-1/2"></div>
                )}
                        ...
                        <div key={activity.id} className="relative flex gap-4 items-start group">
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex justify-between items-start gap-2 mb-1">
                                    ...
                                    <span className="text-[10px] font-black text-text-muted uppercase tracking-wider whitespace-nowrap pt-0.5">
                                        {activity.created_at_human}
                                    </span>
                                </div>
                                <p className="text-[11px] font-medium text-text-muted uppercase tracking-tight">
